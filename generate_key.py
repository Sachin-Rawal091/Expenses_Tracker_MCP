...
async def register_master(email):
    ...
    print(f"\n✅ Master Key Generated for {email}")
    print(f"🔑 Key: {raw_key}")
    print("\n⚠️  SAVE THIS KEY NOW. It will never be shown again.")
    print("   To connect permanently, use: ?user_id=" + email + "&key=" + raw_key)
...
async def create_session(email, master_key, hours=None, device="Unknown"):
    ...
    expiry = datetime.now() + timedelta(hours=hours) if hours else None

    await conn.execute(
        "INSERT INTO sessions (user_id, token_hash, expires_at, device_name) VALUES ($1, $2, $3, $4)",
        user['id'], sess_hash, expiry, device
    )
    await conn.close()

    expiry_str = f"Expires in {hours}h" if hours else "Never Expires (Persistent)"
    print(f"\n✅ Session Token Created ({expiry_str})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--session", action="store_true", help="Create a persistent/temporary session")
    parser.add_argument("--revoke", action="store_true", help="Revoke all sessions")
    parser.add_argument("--email", help="User email")
    parser.add_argument("--key", help="Master key")
    parser.add_argument("--hours", type=int, help="Optional expiry for session")
    ...
    if args.register:
        ...
        if not args.email or not args.key:
            print("❌ Email and Master Key required to create a session.")
        else:
            asyncio.run(create_session(args.email, args.key, args.hours))
    elif args.revoke:
        if not args.email or not args.key:
            print("❌ Email and Master Key required.")
        else:
